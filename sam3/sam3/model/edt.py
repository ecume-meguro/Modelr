...
try:
    ...
    HAS_TRITON = False
    # Mock triton for non-CUDA environments
    class MockTriton:
        def jit(self, func):
            return func
    triton = MockTriton()
...
def edt_triton(data: torch.Tensor):
    ...
    if not HAS_TRITON or not data.is_cuda:
        ...
        device = data.device
        data_cpu = data.detach().cpu().numpy().astype(np.uint8)
        B, H, W = data_cpu.shape
        outputs = []
        for i in range(B):
            # cv2.distanceTransform calculates distance to the closest 0 for non-zero pixels.
            dist = cv2.distanceTransform(data_cpu[i], cv2.DIST_L2, 5)
