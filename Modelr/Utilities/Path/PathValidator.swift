...
class PathValidator {
    ...
    private func setupAllowedDirectories() {
        ...
        if let appSupportBase {
            let currentRoot = appSupportBase.appendingPathComponent(AppConstants.appSupportDirectoryName, isDirectory: true)
            allowedDirectories.insert(currentRoot)
            allowedDirectories.insert(currentRoot.resolvingSymlinksInPath())
            allowedDirectories.insert(currentRoot.appendingPathComponent("Hunyuan3D", isDirectory: true))
            allowedDirectories.insert(currentRoot.appendingPathComponent("checkpoints", isDirectory: true))

            // Allow legacy root during migration/back-compat.
            let legacyRoot = appSupportBase.appendingPathComponent(AppConstants.legacyAppSupportDirectoryName, isDirectory: true)
            allowedDirectories.insert(legacyRoot)
            allowedDirectories.insert(legacyRoot.resolvingSymlinksInPath())
            allowedDirectories.insert(legacyRoot.appendingPathComponent("Hunyuan3D", isDirectory: true))
            allowedDirectories.insert(legacyRoot.appendingPathComponent("checkpoints", isDirectory: true))
        }
        ...
        // Since we are not sandboxed (per project.yml), allow user home directory for convenience
        let homeDir = fileManager.homeDirectoryForCurrentUser
        allowedDirectories.insert(homeDir)
        allowedDirectories.insert(homeDir.resolvingSymlinksInPath())
    }
