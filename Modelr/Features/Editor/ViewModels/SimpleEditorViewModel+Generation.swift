...
extension SimpleEditorViewModel {
    ...
    /// Generate an image from text prompt using T2I (Stable Diffusion)
    func generateT2IImage() {
        guard !t2mPrompt.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            print("[T2I] Empty prompt, skipping generation")
            return
        }

        // Cancel any existing T2I task
        t2iTask?.cancel()

        isGeneratingT2I = true
        t2mProgress = 0
        t2mProgressDetail = "Starting..."
        t2mCurrentStep = .generate

        t2iTask = Task { @MainActor in
            do {
                // Ensure project directory exists
                guard let projectId = self.projectId else {
                    throw AppError.generation("No project ID")
                }

                let projectDir = PathManager.projectDirectory(for: projectId)
                let outputPath = projectDir.appendingPathComponent("t2i_generated.png")

                // Get Python path from inference environment
                let pythonPath = PathManager.inferenceVenvDirectory
                    .appendingPathComponent("bin")
                    .appendingPathComponent("python3")

                // Create T2I process manager
                let t2iManager = T2IProcessManager()

                // Start the server
                try await t2iManager.start(
                    pythonPath: pythonPath,
                    wrapperPath: PathManager.t2iWrapperPath
                )

                // Generate the image
                let result = try await t2iManager.generate(
                    prompt: t2mPrompt,
                    outputPath: outputPath.path,
                    negativePrompt: t2mNegativePrompt,
                    width: 512,
                    height: 512,
                    steps: 20,
                    guidanceScale: 7.5,
                    seed: t2mSeed
                ) { [weak self] stage, progress, detail in
                    Task { @MainActor in
                        self?.t2mProgress = progress
                        self?.t2mProgressDetail = "\(stage): \(detail)"
                    }
                }

                // Stop the server
                await t2iManager.stop()

                // Load the generated image
                if let image = NSImage(contentsOf: URL(fileURLWithPath: result.imagePath)) {
                    t2mGeneratedImage = image
                    t2mGeneratedImagePath = result.imagePath
                    t2mSeed = result.seed

                    // Use this as the input image for the rest of the pipeline
                    inputImage = image

                    // Update project metadata
                    if var metadata = ProjectManager.shared.loadMetadata(for: projectId) {
                        metadata.t2iPrompt = t2mPrompt
                        metadata.t2iNegativePrompt = t2mNegativePrompt
                        metadata.t2iGeneratedImagePath = "t2i_generated.png"
                        metadata.t2iSeed = result.seed
                        try? ProjectManager.shared.saveMetadata(metadata)
                    }

                    print("[T2I] Image generated successfully: \(result.imagePath)")
                }

                isGeneratingT2I = false
                t2mProgress = 1.0
                t2mProgressDetail = "Complete"

            } catch {
                print("[T2I] Generation failed: \(error)")
                isGeneratingT2I = false
                t2mProgress = 0
                t2mProgressDetail = "Failed: \(error.localizedDescription)"
                t2mCurrentStep = .prompt
            }
        }
    }

    /// Cancel T2I generation
    func cancelT2IGeneration() {
        t2iTask?.cancel()
        t2iTask = nil
        isGeneratingT2I = false
        t2mProgress = 0
    ...
    // MARK: - Autonomous Text-to-Model Pipeline

    /// Start the full autonomous text-to-model pipeline:
    /// 1. Generate image from text (T2I)
    /// 2. Auto-detect object with VLM
    /// 3. Segment with SAM
    /// 4. Generate 3D model with Hunyuan
    func startTextToModelPipeline() {
        guard !t2mPrompt.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            print("[T2M Pipeline] Empty prompt, skipping")
            return
        }
        ...
        isGeneratingT2I = true
        t2mProgress = 0
        t2mProgressDetail = "Starting image generation..."

        t2iTask = Task { @MainActor in
            do {
                // Step 1: Generate image from text
                try await runT2IGeneration()

                // Check cancellation
                try Task.checkCancellation()

                // Step 2: Auto-detect object with VLM
                t2mProgressDetail = "Detecting object..."
                try await runVLMAutoDetect()

                // Check cancellation
                try Task.checkCancellation()

                // Step 3: Segment with SAM (select first/best detection)
                t2mProgressDetail = "Segmenting object..."
                try await runSAMSegmentation()

                // Check cancellation
                try Task.checkCancellation()

                // Step 4: Prepare composite and start 3D generation
                t2mProgressDetail = "Starting 3D generation..."
                isGeneratingT2I = false
                prepareCompositeImage()

                // Wait a moment for composite to be ready
                ...
                print("[T2M Pipeline] Cancelled")
                isGeneratingT2I = false
                t2mProgress = 0
                t2mProgressDetail = ""
            } catch {
                print("[T2M Pipeline] Failed: \(error)")
                isGeneratingT2I = false
                t2mProgress = 0
                t2mProgressDetail = "Failed: \(error.localizedDescription)"
                lastError = AppError.generation(error.localizedDescription)
                showErrorAlert = true
                // Return to prompt step on failure
                t2mCurrentStep = .prompt
            }
        }
    }

    /// Stop the entire text-to-model pipeline
    func stopTextToModelPipeline() {
        // Cancel T2I task
        t2iTask?.cancel()
        t2iTask = nil
        isGeneratingT2I = false
    ...
    /// Run T2I generation as part of the pipeline
    private func runT2IGeneration() async throws {
        guard let projectId = self.projectId else {
            throw AppError.generation("No project ID")
        }
        ...
        // Load the generated image
        guard let image = NSImage(contentsOf: URL(fileURLWithPath: result.imagePath)) else {
            throw AppError.generation("Failed to load generated image")
        }
        ...
        t2mSeed = result.seed
        inputImage = image
        inputImagePath = result.imagePath  // Required for VLM auto-detection
        ...
        print("[T2M Pipeline] Image generated: \(result.imagePath)")
    }

    /// Run VLM auto-detection as part of the pipeline
    private func runVLMAutoDetect() async throws {
        guard inputImage != nil, let imagePath = inputImagePath else {
            throw AppError.generation("No input image for VLM detection")
        }

        // Initialize segmentation state - create a default entry if needed
        if segmentations.isEmpty {
            segmentations.append(SegmentationEntry(name: "Object 1"))
            activeSegmentationIndex = 0
        }

        // Initialize the image with SAM backend
        do {
            let size = try await env.setImage(path: imagePath)
            imagePixelSize = size
            isImageInitializedWithSAM = true
            print("[T2M Pipeline] Image initialized with SAM: \(size)")
        } catch {
            print("[T2M Pipeline] Failed to initialize image with SAM: \(error)")
            throw AppError.generation("Failed to initialize image with SAM")
        }

        // Use the existing auto-detect functionality
        triggerAutoDetection()

        // Wait for detection to complete (isAutoDetecting flag or segmentations appear)
        var attempts = 0
        while (segmentations.first?.allMasks.isEmpty == true) && attempts < 60 {
            try await Task.sleep(nanoseconds: 200_000_000) // 0.2s
            attempts += 1
        }

        guard !segmentations.isEmpty, segmentations.first?.allMasks.isEmpty == false else {
            throw AppError.generation("VLM/SAM failed to detect and segment object")
        }

        print("[T2M Pipeline] VLM detected \(segmentations.count) object(s)")
    }

    /// Run SAM segmentation as part of the pipeline
    private func runSAMSegmentation() async throws {
        // Select the first/best segmentation automatically
        guard let firstSegmentation = segmentations.first else {
            throw AppError.generation("No segmentations available")
        }

        // Select the first mask from the first detection (index 0)
        if !firstSegmentation.allMasks.isEmpty {
            selectMask(at: 0, for: 0, addToSelection: false)
        }

        // Wait a moment for mask to be processed
        try await Task.sleep(nanoseconds: 300_000_000) // 0.3s

        // Merge masks and create editable mask
        if editableMaskImage == nil {
            let masks = segmentations.flatMap { $0.selectedMasks }
            if !masks.isEmpty {
                editableMaskImage = ImageService.shared.mergeMasks(masks)
            }
        }

        guard editableMaskImage != nil else {
            throw AppError.generation("Failed to create segmentation mask")
        }

        print("[T2M Pipeline] SAM segmentation complete")
    }
}
